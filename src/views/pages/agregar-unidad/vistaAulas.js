...
const VistaAulas = () => {
  ...
  const [showWarnngMessage, setShowWarningMessage] = useState({
    show: false,
    message: '',
  })
  ...
  const handleAgregarAula = async () => {
    try {
      ...
      if (isNaN(capacidad)) {
        ...
        showWarnngMessage((prev) => {
          return {
            ...prev,
            show: true,
            message: 'Por favor, ingrese una capacidad válida.',
          }
        })
        return
      }
  ...
  const handleGuardarEdicionAula = async () => {
    try {
      ...
      const todasLasAulas = await getAulas() // Obtener todas las aulas registradas en la base de datos
      const aulaExistente = todasLasAulas.find(
        (aula) => aula.nombreAulas === nombreAulaEditando && aula.id !== aulaEditandoId,
      ) // Verificar si el nombre del aula editada ya existe, excluyendo el aula que se está editando
  ...
  return (
    <CContainer>
      <CCard>
          ...
          {mostrarFormulario && (
            <div className="mb-3">
              <CRow>
                ...
                <CCol xs="auto">
                  <CButton color="primary" onClick={handleAgregarAula}>
                    Guardar
                  </CButton>
                </CCol>
              </CRow>
            ...
            <CTableBody>
              {aulas.map((aula, index) => (
                <CTableRow key={index}>
                  ...
                  <CTableHeaderCell>
                    {aulaEditandoId === aula.id ? (
                      <CButton color="primary" onClick={handleGuardarEdicionAula}>
                        Guardar
                      </CButton>
                    ) : (
                      <CButton color="primary" onClick={() => handleEditarAula(aula.id)}>
                        <CIcon icon={cilPen} />
                      </CButton>
                    )}
                  </CTableHeaderCell>
        ...
        <CRow>
          <CCol>
            {showWarnngMessage.show && <CAlert color="success">showWarnngMessage.message</CAlert>}
            {/* {showDuplicate && (
